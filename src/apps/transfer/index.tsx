...
import { useDelegateAllowance, useInputAddress, useInputNumber, useMultisig, useSafeInfo } from '@mimir-wallet/hooks';
...
function Transfer() {
  ...
  const info = useSafeInfo(multisig?.address);
  ...
  const [{ loading }, handleClick] = useAsyncFn(async () => {
    if (!to || !current || !amount || !info || !multisig || !isAddress(to)) {
      return;
    }

    const tx = buildSafeTransaction(to, info[0], { value: parseEther(amount) });
  ...
  }, [amount, current, info, multisig, navigate, openTxModal, to]);
      ...
      <div className='space-y-4'>
        ...
        {allowance && allowance[0] - allowance[1] > 0n && (
          <div className='flex justify-between'>
            ...
            <span>
              <FormatBalance value={allowance[0] - allowance[1]} prefix={<>Spend Limit:</>} />
            </span>
          </div>
        )}
        <div className='flex gap-3'>
          ...
          {allowance && allowance[0] - allowance[1] > 0n && useSpendLimit ? (
            <ButtonEnable isLoading={transferLoading} onClick={handleTransfer} fullWidth radius='full' color='primary'>
              Submit
            </ButtonEnable>
          ) : (
            <ButtonEnable isLoading={loading} onClick={handleClick} fullWidth radius='full' color='primary'>
              Submit
            </ButtonEnable>
          )}
        </div>
